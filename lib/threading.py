...
class Semaphore:
	__handle: Ptr[None]  # HANDLE on Windows, Ptr[posix.semaphore.sem_t] on POSIX

	@compiler.target( os = 'windows' )
	def __init__( self, initial_count: u32 = 0 ) -> None:
		from windows.kernel32 import CreateSemaphoreW
		with compiler.panic_arithmetic( 'Semaphore.__init__: initial_count overflow' ):
			handle: Ptr[None] = CreateSemaphoreW( None, i32( initial_count ), i32( 2147483647 ), None )
		if handle is None:
			sys.panic( 'Semaphore.__init__: CreateSemaphoreW failed' )
		self.__handle = handle

	@compiler.target( os = not 'windows' )
	def __init__( self, initial_count: u32 = 0 ) -> None:
		from posix.semaphore import sem_init, sem_t
		sem_size: usize = compiler.sizeof( sem_t )
		raw: Ptr[u8] = sys._alloc( sem_size )
		sys.memzero( raw, sem_size )
		self.__handle = raw
		result: i32 = sem_init( self.__handle, 0, initial_count )
		if result != 0:
			sys.panic( 'Semaphore.__init__: sem_init failed' )

	@compiler.target( os = 'windows' )
	def post( self ) -> None:
		from windows.kernel32 import ReleaseSemaphore
		ReleaseSemaphore( self.__handle, 1, None )

	@compiler.target( os = not 'windows' )
	def post( self ) -> None:
		from posix.semaphore import sem_post
		sem_post( self.__handle )

	@compiler.target( os = 'windows' )
	def wait( self ) -> None:
		from windows.kernel32 import WaitForSingleObject, INFINITE
		WaitForSingleObject( self.__handle, INFINITE )

	@compiler.target( os = not 'windows' )
	def wait( self ) -> None:
		from posix.semaphore import sem_wait
		sem_wait( self.__handle )
	...
	@compiler.target( os = not 'windows' )
	def __del__( self ) -> None:
		from posix.semaphore import sem_destroy
		sem_destroy( self.__handle )
